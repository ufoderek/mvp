 ...
 * All Right Reserved
 *
 * the ARM PL180 multi-media card interface module of MVP emulator
 *
...
/* the constructor */
MCI::MCI(sc_module_name name, uint32_t mapping_size): ahb_slave_if(mapping_size), sc_module(name)
{
}
...
bool MCI::local_access(bool write, uint32_t addr, uint32_t& data, unsigned int length)
{
    bool success;
    ...
    if (write) {
        ...
        printd(d_mci, "write to phy address %u: %u", addr, data);
    }

    else {
        success = this->read(&data, local_address, length);
        printd(d_mci, "read from phy address %u: %u", addr, data);
    }
